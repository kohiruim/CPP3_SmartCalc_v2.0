...
bool CalcView::validNum(std::string input) {
  bool result = true;
  int count_dot = 0;
  if (input.empty()) {
    result = false;
  } else {
    for (int i = 0; i < (int)input.length() && result; i++) {
      if (input[i] == '.') {
        count_dot++;
      }
      if (!((input[i] == '.' || std::isdigit(input[i])) && count_dot < 2)) {
        result = false;
      }
      if ((input.length() > 1) && (i == 0) &&
          (input[i] == '+' || input[i] == '-') && (count_dot < 2)) {
        result = true;
      }
    }
  }
...
void CalcView::onEqualsClicked() {
  ...
  } else if (input.contains('X') != 0 && !validNum(x.toStdString())) {
    ui_->xValue->setText("что-то не так");
  } else if (checkLastElement(ui_->input->text())) {
    std::string result = controller_.calc(input.toStdString(), x.toStdString());
    ui_->output->setText(QString::fromLocal8Bit(result.c_str()));
  }
}

bool CalcView::checkGraphInput(std::vector<std::string> values) {
  bool result = true;
  for (int i = 0; i < (int)values.size() && result; i++) {
    if (!validNum(values[i]) || abs(std::stod(values[i])) > 1000000) {
      result = false;
    }
  }
  return result;
...
void CalcView::onGraphClicked() {
  if (checkLastElement(ui_->input->text())) {
    ...
    std::vector<std::string> values = {
        ui_->x_start->text().toStdString(), ui_->x_end->text().toStdString(),
        ui_->y_start->text().toStdString(), ui_->y_end->text().toStdString()};
    ...
    double x_start = std::stod(values[0]);
    double x_end = std::stod(values[1]);
    double y_start = std::stod(values[2]);
    double y_end = std::stod(values[3]);
    ...
    controller_.makeGraph(x, y, x_start, x_end, y_start, y_end,
                          ui_->input->text().toStdString());
    ui_->widget->addGraph();
    ui_->widget->graph(0)->addData(QVector<double>(x.begin(), x.end()),
